...
class DebugTailCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $fileName = $this->option('file') ?? ($logStyle === 'single' ? 'agent_debug.log' : 'agent_debug-' . date('Y-m-d') . '.log');
        $filePath = $logPath . '/' . $fileName;

        render('
            <div class="px-1 py-1 bg-red-600 text-white font-bold mb-1">🚀 Laravel Agent-Debugger Live Stream</div>
        ...
        while (true) {
            ...
            if (str_contains($line, '================================================================================')) {
                if (trim($buffer) !== '================================================================================') $this->formatRequestBlock($buffer);
                $buffer = '';
            }
        }
        fclose($handle);
    ...
    {
        $lines = explode("\n", $block);
        $requestLine = $metaLine = '';
        $crashed = false;
        $warnings = $queries = $exceptions = [];

        foreach ($lines as $line) {
            $trimmed = trim($line);
            if (empty($trimmed) || str_starts_with($trimmed, '=====')) continue;
            if (str_contains($trimmed, 'REQUEST:')) $requestLine = str_replace('REQUEST:', '', $trimmed);
            elseif (str_contains($trimmed, 'IP:')) {
                $metaLine = $trimmed;
                if (str_contains($trimmed, 'Response Status: 5') || str_contains($trimmed, 'CRASHED')) $crashed = true;
            } elseif (str_contains($trimmed, '⚠️ WARNING:')) $warnings[] = $trimmed;
            elseif (str_contains($trimmed, 'select ') || str_contains($trimmed, 'update ') || str_contains($trimmed, 'insert ') || str_contains($trimmed, 'delete ')) $queries[] = $trimmed;
            elseif (str_contains($trimmed, 'Class: ') || str_contains($trimmed, 'Message: ') || str_contains($trimmed, 'File: ')) $exceptions[] = $trimmed;
        }

        $filter = $this->option('filter');
        if ($filter) {
            $filter = strtolower($filter);
            if ($filter === 'errors' && !$crashed && empty($exceptions)) return;
            if ($filter === 'n+1' && !array_filter($warnings, fn($w) => str_contains(strtolower($w), 'n+1'))) return;
            if ($filter === 'slow' && !array_filter($queries, fn($q) => str_contains($q, 'slow query'))) return;
        }
        ...
        foreach ($warnings as $warning) render('<div class="bg-yellow-900 text-yellow-100 px-2 py-1 text-xs font-bold mb-1 border-l-4 border-yellow-500">' . htmlspecialchars($warning) . '</div>');
        ...
        if (!empty($exceptions)) {
            ...
            foreach ($exceptions as $ex) render('<div>' . htmlspecialchars($ex) . '</div>');
            render('</div>');
        }

        if (!empty($queries)) {
            $slowCount = count(array_filter($queries, fn($q) => str_contains($q, 'SLOW QUERY')));
            $slowText = $slowCount > 0 ? " (<span class='text-red-400 font-bold'>{$slowCount} SLOW</span>)" : "";
            render('<div class="text-gray-500 text-xs italic pl-2">⚡ Executed ' . count($queries) . ' database queries' . $slowText . '</div>');
        }
    }
}
